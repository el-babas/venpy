...
{
   public partial class SALE003LView : UserControl, ISALE003LView
   {
      #region [ PROPERTIES ]
      ...
      public SALE003LView(TabPage p_tabPageControl)
      {
      ...
      public void LViewLoad()
      {
         try
         {
            dtpPROF_DateEnd.Value = DateTime.Now;
            dtpPROF_DateBegin.Value = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
            txtPROF_Number.Text = null;
            chkPROF_OnlyActive.Checked = true;
            CreateGridColumns();
         }
         catch (Exception ex)
      ...
      public void Search()
      {
         ...
         {
            CleanDataGridView();
            Presenter.Search(txtPROF_Number.Text, dtpPROF_DateBegin.Value, dtpPROF_DateEnd.Value, chkPROF_OnlyActive.Checked);
            FillDataGridView();
         }
         catch (Exception ex)
      ...
      private void CreateGridColumns()
      {
         try
         {
            ...
            dgvItems.AddImageColumn("ANULAR", "Cancel", "Cancel", 75, Properties.Resources.cancel_16x16, null, DataGridViewContentAlignment.MiddleCenter);
            dgvItems.AddTextBoxColumn("NÚMERO", "PROF_Number", "PROF_Number", 100, true, null, DataGridViewContentAlignment.MiddleLeft);
            dgvItems.AddTextBoxColumn("FEC. EMISION", "PROF_Date", "PROF_Date", 150, true, "dd/MM/yyyy", DataGridViewContentAlignment.MiddleCenter);
            dgvItems.AddTextBoxColumn("MONEDA", "TBLE_NameMND", "TBLE_NameMND", 150, true, null, DataGridViewContentAlignment.MiddleLeft);
            dgvItems.AddTextBoxColumn("CLIENTE", "PROF_CustomerName", "PROF_CustomerName", 350, true, null, DataGridViewContentAlignment.MiddleLeft);
            dgvItems.AddTextBoxNumericColumn("PRECIO TOTAL", "PROF_TotalPrice", "PROF_TotalPrice", 100, false, DataGridViewContentAlignment.MiddleRight, 20, Presenter.NumberDecimals);
            dgvItems.AddTextBoxColumn("ESTADO", "PROF_NameStatus", "PROF_NameStatus", 100, true, null, DataGridViewContentAlignment.MiddleLeft);
            dgvItems.AddTextBoxColumn("CÓDIGO", "PROF_Code", "PROF_Code", 100, true, null, DataGridViewContentAlignment.MiddleCenter);

            dgvItems.AddTextBoxColumn("CÓDIGO EMPRESA", "BUSI_Code", "BUSI_Code", 100, true, null, DataGridViewContentAlignment.MiddleCenter);
      ...
      private void SelectItem()
      {
         try
         {
            if (Presenter != null)
            {
               if (BSItems != null && BSItems.Current != null && BSItems.Current is VENPY_Proformas)
               { Presenter.ItemGridView = ((VENPY_Proformas)BSItems.Current); }
               else
